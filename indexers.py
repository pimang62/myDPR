...
class DenseIndexer(object):
    ...
    """---------------file_name 임의로 추가-----------------"""
    def serialize(self, file: str, file_name: str):
        logger.info("Serializing index to %s", file)

        if os.path.isdir(file):
            index_file = os.path.join(file, f"{file_name}.dpr")
            meta_file = os.path.join(file, f"{file_name}_meta.dpr")
        else:
    ...
    def get_files(self, path: str, file_name: str):
        if os.path.isdir(path):
            index_file = os.path.join(path, f"{file_name}.dpr")
            meta_file = os.path.join(path, f"{file_name}_meta.dpr")
        else:
            index_file = path + ".{}.dpr".format(self.get_index_name())
            meta_file = path + ".{}_meta.dpr".format(self.get_index_name())
        return index_file, meta_file

    def index_exists(self, path: str, file_name: str):
        index_file, meta_file = self.get_files(path, file_name)
        return os.path.isfile(index_file) and os.path.isfile(meta_file)

    def deserialize(self, path: str, file_name: str):
